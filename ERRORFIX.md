...
## 2026-02-02 12:21 +0800｜TUI：Hat Graph Radar 生成导致 `ralph run --tui` 启动前长时间卡住

### 现象
- `ralph run --tui` 在进入 TUI（alternate screen）之前卡住很久：
  - 体感是“长时间无 UI/黑屏”，然后才出现界面。

### 根因
- Radar 复用了 `beautiful-mermaid-rs` 的 Mermaid→ASCII 渲染（QuickJS + eval 大 bundle）。
- 该渲染在本机耗时非常夸张（量化见 notes）：
  - release 约 22 秒
  - debug 约 87 秒
- 且旧实现会渲染两次（compact + full），把启动路径的阻塞放大。

### 修复
- **不再在 Radar 路径中做 Mermaid→ASCII（QuickJS）渲染**。
- 改为直接显示 Mermaid 源码文本（轻量且信息完整）：
  - compact：edges-only（关键连线概览）
  - full：完整 Mermaid（含节点 label）
- 这样 Radar 仍可用，但不会拖慢 `--tui` 的启动。
