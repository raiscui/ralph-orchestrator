...
## [2026-05-14 17:47:00] [Session ID: omx-1778510695653-7pd7o2] 后续计划: Phase 4 live runtime capability invocation

- 背景:
  - Phase 3 已经把 `ralph tools capability invoke` 的 isolated child/micro-run artifacts 串到 `.ralph/evidence-index.jsonl`。
  - Phase 3.1 已经补上 `ralph tools capability inspect <invocation_id>` 查询 UX。
- 下一条建议:
  - 创建 OpenSpec change,推进 Phase 4: 让真实 `ralph#1` 在 parent run 中选择并触发 capability invocation。
  - 执行仍应使用 isolated child/micro-run,把 result/failure artifact 回传 parent run。
  - parent topology 仍不得热改。
- 建议验收:
  - parent run 能产生 capability invocation id。
  - child/micro-run artifact、`.ralph/events.jsonl`、`.ralph/evidence-index.jsonl` 都可通过 Phase 3.1 inspect UX 查到。
  - parent run 能消费 structured capability result/failure,而不是只把文件落盘。
- 暂不本轮实施原因:
  - Phase 4 会进入 live runtime 语义,需要独立 OpenSpec 和更强 dogfood gate。
  - 先稳定 evidence inspect UX,可以降低 Phase 4 调试成本。
