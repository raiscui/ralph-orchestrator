...
## 2026-02-07 12:37 +0800 | 修复 `AsciiRenderOptions` 字段演进导致的 `cargo build --release` 失败

- 问题现象:
  - `cargo build --release` 报 `E0063 missing field max_width in initializer of AsciiRenderOptions`。
  - 触发文件: `crates/ralph-cli/src/hats.rs`。
- 根因分析:
  - `beautiful-mermaid-rs` 的 `AsciiRenderOptions` 新增了 `max_width` 字段。
  - `hats.rs` 中两处使用“完整字面量初始化”未包含该字段,编译器报结构体字段缺失。
- 实施修复:
  - 在两处初始化补上 `..Default::default()`。
  - 这样 `max_width` 自动走默认值(`None`,由库侧回退到 200),并降低未来字段演进再次触发编译错误的概率。
- 代码变更:
  - `crates/ralph-cli/src/hats.rs`
- 验证结果:
  - `cargo build --release` 通过。
  - `cargo test` 全量通过(含 `ralph-core` smoke 与 doctests)。

## 2026-02-07 12:52 +0800 | 继续任务: 方案A 统一 `AsciiRenderOptions` 构造入口

- 背景:
  - 上一轮先用方案B快速修复了 `max_width` 缺失导致的编译失败。
  - 本轮按“继续”执行后续优化,将分散初始化收敛为统一构造入口。
- 实施:
  - 在 `crates/ralph-cli/src/hats.rs` 新增:
    - `unicode_render_options()`
    - `ascii_render_options()`
    - `compact_unicode_render_options()`
  - 将 `render_hat_dag_via_mermaid` 与 `render_hat_graph_radar_ascii` 的字面量初始化替换为上述构造函数。
  - 新增测试 `test_ascii_render_option_builders_keep_future_fields_on_default` 锁定未来字段承接行为。
- 验证:
  - `cargo build --release` 通过。
  - `cargo test` 全量通过(含 smoke/doctests)。
  - `cargo fmt --check` 通过。

## 2026-02-07 13:08 +0800 | 修复 `--format mermaid` 下 Ralph 边标签过长导致图过宽

- 现象:
  - physical 视图里,`Hat_* -> Hat_ralph` 多 topic 被折叠成单条边,形成超长 label。
  - 典型输出: `experiment.complete / integration.applied / integration.blocked / integration.rejected`。
- 根因:
  - `generate_mermaid_string_physical` 对 Ralph 相关边统一执行折叠(`join(" / ")`)。
  - 该策略对 TerminalPretty 有稳定性价值,但对 Strict mermaid 输出会明显拉宽图。
- 修复策略:
  - 分模式处理:
    - `MermaidLabelMode::Strict`: 不折叠 Ralph 多 topic 边,一 topic 一条边。
    - `MermaidLabelMode::TerminalPretty`: 保留原折叠策略,维持 ASCII/Unicode 渲染稳定性。
- 代码位置:
  - `crates/ralph-cli/src/hats.rs:936`
  - `crates/ralph-cli/src/hats.rs:1406`
  - `crates/ralph-cli/src/hats.rs:1438`
- 验证:
  - 定向测试通过:
...
## 2026-02-07 13:21 +0800 | 优化默认 `ralph hats graph` 长边标签: TerminalPretty 自动摘要

- 现象:
  - `ralph hats graph --format mermaid` 已拆分 Ralph 多 topic 边,可读性正常。
  - 但默认 `ralph hats graph`(Unicode/ASCII 路径,TerminalPretty) 仍会显示超长标签,例如 `experiment.complete / integration.applied / ...`。
- 根因:
  - TerminalPretty 仍保留“单边折叠 + 全量 topic 拼接”策略,导致标签长度无上限。
- 处理:
  - 保持 TerminalPretty 的“单边折叠”(不破坏渲染稳定性)。
  - 新增摘要函数 `summarize_terminal_pretty_collapsed_topics`:
    - 短标签: 保留完整 `a / b / c`
    - 长标签: 压缩为 `first_topic / +N more`
- 关键位置:
  - `crates/ralph-cli/src/hats.rs:960`
  - `crates/ralph-cli/src/hats.rs:989`
- 回归测试:
  - `test_generate_mermaid_string_physical_terminal_pretty_keeps_ralph_topic_collapsed`
  - `test_summarize_terminal_pretty_collapsed_topics_keeps_short_label`
- 验证:
  - `cargo build --release` 通过。
  - `cargo test` 全量通过(含 smoke/doctests)。
  - `cargo fmt --check` 通过。
