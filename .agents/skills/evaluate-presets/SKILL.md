...
```bash
./tools/evaluate-preset.sh tdd-red-green Codex
```

**Evaluate all presets:**
```bash
./tools/evaluate-all-presets.sh Codex
```

**Arguments:**
- First arg: preset name (without `.yml` extension)
- Second arg: backend (`Codex` or `kiro`, defaults to `Codex`)
...
```
Bash tool with:
  command: "./tools/evaluate-preset.sh tdd-red-green Codex"
  timeout: 600000
...
```bash
./tools/evaluate-preset.sh <fixed-preset> Codex
```
