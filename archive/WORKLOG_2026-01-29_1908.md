...
## 2026-01-29 14:28 +0800｜并行输出：stderr 默认折叠 + 灰色显示

### 我做了什么
- 新增 CLI 开关：`ralph run --show-stderr` / `ralph resume --show-stderr`。
  - 默认行为：并行模式下 stderr（`:err:`）行会被折叠/隐藏，减少后端日志/回显噪音。
- stderr 行样式优化：
  - 日志模式：stderr 行使用 ANSI 灰色（`colors::GRAY` / `\x1b[90m`）。
  - 并行 TUI：stderr 行使用 `Color::DarkGray`。

### 影响范围
- `crates/ralph-cli/src/main.rs`：新增 `--show-stderr` 参数并透传到并行 runner。
- `crates/ralph-cli/src/parallel_runner.rs`：默认过滤 stderr；开启时灰色输出；首次出现 stderr 且被隐藏时给一次提示。
- `crates/ralph-cli/src/display.rs`：补充 `colors::GRAY` 常量。
- `crates/ralph-tui/src/state/parallel.rs`：stderr 行 DarkGray 渲染。

### 验证
- `cargo test` ✅ 全部通过。

## 2026-01-29 17:10 +0800｜并行 TUI：Targets/Gate chips + 默认消息定向 + human.message 默认订阅

### 我做了什么
- TUI（human-in-loop）交互补齐：
  - Chat 底部渲染 Targets chips（全实例），点击即可切换 `selected_instance`。
  - Gate 列表行可点击：设置 `selected_gate`，并联动切换 `selected_instance = requested_by`。
  - Gate 详情区展示关键信息，并提供 `!approve` / `!deny` / `!resolve` actions chips（点击只预填输入框，不自动发送）。
  - Chat 提交 human.message 时：若未写 `@<HatInstanceId>`，则默认定向到当前 `selected_instance`（避免意外 broadcast）。
- 并行运行时订阅语义补齐：
  - `parallel.enabled: true` 时，所有 hats 运行时视为已订阅 `human.message`（仅用于 strict target 校验，不改变 fanout）。

### 影响范围
- `crates/ralph-tui/src/app.rs`：Targets/Gate chips 渲染与 hit-test；默认目标定向；对应单测。
- `crates/ralph-tui/src/state/parallel.rs`：新增 `selected_gate` 与 `select_instance_by_id`。
- `crates/ralph-tui/src/widgets/help.rs`：帮助页补充 chips 用法提示。
- `crates/ralph-core/src/parallel/supervisor/routing.rs`：strict target 校验里补齐 human.message 默认订阅。
- `crates/ralph-core/src/parallel/supervisor/routing_tests.rs`：新增回归测试覆盖默认订阅。
- `openspec/changes/tui-mouse-chat-and-clean-exit/tasks.md`：勾选完成 tasks 7-9。

### 验证
- `cargo test` ✅ 全量通过。

## 2026-01-29 16:03 +0800｜TUI：鼠标点选实例/输出框选/Chat 多行输入/q 退出触发 interrupt（tui-mouse-chat-and-clean-exit）

### 你最初的需求原文（摘录）
1. "tui 要支持鼠标点选 当前的实例 , 支持 类似文字编辑器一样选择输出的多行多个文字 , 支持 点 chat 进入到chat输入 , chat 输入要像一个正常的输入窗口,类似终端,要有提示符 要能通过鼠标 和键盘支持 提示符的左右上下移动 , 要支持鼠标 键盘 框选 ,支持 shift +回车 换行"
2. "tui 退出时 要退出所有cli进程"

### 我做了什么（实现）
- 并行 TUI 增加鼠标 hit-test + 焦点切换：
  - 点击实例列表行 → 切换当前实例（影响 Output 面板渲染）。
  - 点击 Output → 聚焦 Output，并支持输出框选。
  - 点击 Chat 输入区 → 聚焦 Chat，并把光标定位到点击位置。
  - 鼠标滚轮继续支持 Output 滚动。
- 输出视图支持“多行框选”（屏幕坐标）：
  - 鼠标 Down/Drag/Up 形成 selection。
  - Shift+方向键可扩展 selection。
  - Esc 清空 selection。
  - `ContentPane` 对选中区域使用蓝色背景高亮。
- Chat 输入区支持“正常终端输入窗口”的多行编辑：
  - 提示符：首行 `>`，后续行 `|`。
  - Shift+Enter 换行；Enter 提交；方向键移动；Shift+方向键选择；鼠标点击定位；鼠标拖拽框选并替换选区。
  - `parse_chat_submit` 支持多行 payload：仅解析第一行的 `@instance` / `!command` 前缀，其余行原样保留。
- 退出语义（`q`）：
  - 串行/并行 TUI 按 `q` 会触发 `interrupt_tx`，让上层 runner 进入既有的“终止并清理子进程”路径（目标：不留 worker/CLI 残留）。

### 我做了什么（测试与修复）
- 修复测试编译失败：`crates/ralph-tui/tests/common/mod.rs` 从旧字段 `chat_input` 迁移到 `chat_editor`，并对齐 3 行输入区布局。
- 清理 warning：
  - `crates/ralph-tui/src/state/parallel.rs`：去掉无意义的 `idx` 赋值（避免 `unused_assignments`）。
  - `crates/ralph-tui/src/app.rs`：精简 `ParallelLayoutSnapshot` 字段（避免 `dead_code`）。

### 验证（硬证据）
- `cargo test -p ralph-tui` ✅
- `cargo test` ✅（workspace 全量）
- `cargo test -p ralph-core smoke_runner` ✅（replay smoke tests）

### 后续建议
- 如果你希望“默认消息”自动发给当前选中的实例（避免每次手写 `@writer#2`），可以做一个小改良：
  - Chat 聚焦时提供快捷键插入 `@<selected_instance_id> ` 前缀（保持显式路由语义，同时更省手）。

## 2026-01-29 15:30 +0800｜答疑：parallel TUI 的 chat 如何触发？回复后如何确定“发给谁”？

### 我怎么确认的
- 我沿着“键盘输入 → TUI 写外部事件 → Supervisor 读外部事件 → 路由 → 投递到实例”的链路，把关键点都在代码里定位出来。

### 结论（最重要的两点）
- chat 的“激发/触发”：不是某个神秘开关，而是 **TUI 底部 `Chat / Gates` 面板拿到焦点后**，`Enter` 提交输入，写入外部事件 JSONL（topic=`human.message`）。
  - 入口：`crates/ralph-tui/src/app.rs:260`
  - 解析：`crates/ralph-tui/src/chat.rs:29`
  - 落盘：`crates/ralph-tui/src/external_event_writer.rs:20`
- “发给谁”：由 **Supervisor 路由层**决定。
  - `@writer#2 hello` → 写入 `target_instance=writer#2`，Supervisor 优先“实例直达”投递（并做严格校验）：`crates/ralph-core/src/parallel/supervisor/routing.rs:165`
  - `hello`（无 `@`）→ 不带 `target_instance`，走 TopicContract/trigger-driven 默认路由；无人订阅则兜底升级给 `ralph#1`：`crates/ralph-core/src/parallel/supervisor/routing.rs:442`
  - `!approve/!deny/!resolve ...` → 写 `gate.resolve`，Supervisor 用 `gate_id -> requested_by` 自动回送给原请求者：`crates/ralph-core/src/parallel/supervisor/gate.rs:55`

### 下一步建议
- 如果你希望“默认消息”自动发给当前选中的实例（而不是必须手写 `@writer#2`），可以在 TUI 里做一个很小的改良：
  - Chat 聚焦时，按一个快捷键自动插入 `@<selected_instance_id> ` 前缀（避免误投递，也不改变现有语义）。

## 2026-01-29 14:42 +0800｜OpenSpec：创建 proposal（tui-mouse-chat-and-clean-exit）

### 我做了什么
- 生成并写入 `openspec/changes/tui-mouse-chat-and-clean-exit/proposal.md`。
- proposal 中明确本次变更以“修改既有能力”为主：
  - `parallel-supervisor-tui`
  - `supervisor-human-chat-gate`
  - `parallel-hat-instances`

### 状态
- `openspec status --change "tui-mouse-chat-and-clean-exit"`：进度 `1/4`。
- 已解锁：`design`、`specs`。

### 下一步
- 继续执行 `/opsx:continue` 将创建下一个工件（通常是 `design` 或 `specs`，以 status 的第一个 ready 为准）。

## 2026-01-29 15:02 +0800｜OpenSpec：/opsx:ff 快进生成全部工件（tui-mouse-chat-and-clean-exit）

### 我做了什么
- 生成 `design.md`：`openspec/changes/tui-mouse-chat-and-clean-exit/design.md`
- 生成 3 份 delta specs（对应 proposal 的 Modified Capabilities）：
...
- 生成 `tasks.md`：`openspec/changes/tui-mouse-chat-and-clean-exit/tasks.md`

### 状态
- `openspec status --change "tui-mouse-chat-and-clean-exit"`：进度 `4/4`，All artifacts complete ✅

### 下一步
- 可以运行 `/opsx:apply` 开始实现该 change。

## 2026-01-29 14:33 +0800｜OpenSpec：新建 change（tui-mouse-chat-and-clean-exit）

### 我做了什么
- 从你的需求描述派生 change 名称：`tui-mouse-chat-and-clean-exit`。
- 使用默认 workflow schema（`spec-driven`）创建 change 脚手架：
  - `openspec new change "tui-mouse-chat-and-clean-exit"`
  - 目录：`openspec/changes/tui-mouse-chat-and-clean-exit/`
- 查看 artifact 状态并定位首个可执行工件：
  - `openspec status --change "tui-mouse-chat-and-clean-exit"` → `0/4`，首个 ready 为 `proposal`
- 只获取首个工件的 instructions 模板（未创建任何工件文件）：
  - `openspec instructions proposal --change "tui-mouse-chat-and-clean-exit"`

### 下一步（等待你确认）
- 进入 `/opsx:continue`，我会基于模板起草 `proposal.md`，但仍然不会进入实现代码阶段。

## 2026-01-29 14:38 +0800｜并行收尾：退出前 drain 状态，消除 StateChanged send 失败 warning

### 我做了什么
- 修复并行模式在自然结束时的“收尾 warning”：
  - Supervisor 结束前会先发 cancel/shutdown，再短暂 drain instance 事件并等待终态（Done/Failed）。
  - 这样避免 `instance_rx` 被提前 drop，导致 HatInstance 在 `set_state(Done)` 时 `send(StateChanged)` 失败。
- 新增回归测试，确保 `ParallelSupervisor::run` 返回时实例状态已收敛到终态。

### 影响范围
- `crates/ralph-core/src/parallel/supervisor.rs`：新增 shutdown-drain（等待实例终态），并将 `.ralph/current-events` 路径解析改为相对 `workspace_root`。
- `crates/ralph-core/src/parallel/supervisor/routing_tests.rs`：新增 `supervisor_run_waits_for_instances_to_reach_terminal_state_on_shutdown`。
