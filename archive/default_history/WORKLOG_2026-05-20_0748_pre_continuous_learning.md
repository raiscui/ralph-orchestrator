...
## [2026-05-17 21:50:00] [Session ID: omx-1779004640353-blcixq] 任务名称: 并行 TUI evidence/status 与 Output act
### 任务内容
- Output 底部新增 evidence/status,显示 events、evidence-index、agents、record-session。
- `act` 从 Footer 挪到 Output 最下方,raw/audit 视图保留 activity 审计行。
### 验证证据
- Focused tests: output status、evidence、activity、audit、path injection passed。
- `cargo test --quiet`、`cargo fmt --all -- --check` 和 `git diff --check`: passed。
### 总结感悟
- 状态摘要靠近 Output,审计流留给 Audit,能同时不漏信息和不挤正文。
