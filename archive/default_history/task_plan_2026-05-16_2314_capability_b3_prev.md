...
状态:
- **当前任务已完成** - 进入本地提交与下一条产品线选择。
## [2026-05-16 20:53:00] [Session ID: omx-1778510695653-7pd7o2] 新任务启动: 方向B.3 - structured failure classes for parent branching policy

目标:
- 继续 capability invocation 的 failure 演进线,把 `capability.failed` 从“只有错误文本”推进到“带结构化失败类型”。
- 先证明 parent run 可以依据结构化 `failure_class` 做分支决策,而不是靠模糊字符串猜测。

阶段计划:
- [ ] 阶段1: 盘点现有 failure payload 与最适合结构化的失败分类
- [ ] 阶段2: 写窄 OpenSpec change,定义 `failure_class` 与 parent branching policy 的最小边界
- [ ] 阶段3: 实现结构化 failure class,并让现有 fallback gate 按 class 断言
- [ ] 阶段4: 补 focused gate / unit tests / stable spec sync
- [ ] 阶段5: 收口验证、归档与本地提交

当前假设:
- B.3 最值钱的不是立刻做很多 fallback 分支,而是先把 parent policy 的判断输入结构化。
- 当前最小可行产品面是 `failure_class=invalid_capability_id`,并用它来驱动已有 fallback branch。
- 备选解释是: 仅靠现有 `invocation_id` / `capability_id` / `error` 已足够,无需新字段。若后续代码阅读发现现有字段已经能稳定覆盖所有分支判断,则不新增 class 字段。

推翻当前假设的证据:
- 如果现有 failure payload 已经有稳定、明确、无需解析字符串的单一分类源,那就不应新增 `failure_class`。
- 如果新增 class 会明显扩大兼容面而没有带来更稳定的 parent policy 输入,则应回退到仅补文档和测试。

状态:
- **目前在阶段1** - 先做只读勘查,确认结构化 failure class 是否值得做。
## [2026-05-16 21:04:00] [Session ID: omx-1778510695653-7pd7o2] 阶段推进: 继续 B.3 半成品实现

当前现场:
- `CapabilityFailureClass` 已经被加入 `crates/ralph-core/src/capability.rs`。
- 还未补齐所有 failed record 构造点、re-export、OpenSpec change、focused gate 断言和验证。

即将执行:
- 先运行 focused compile/test 暴露所有缺字段与导出漏点。
- 按编译错误补齐构造点,避免凭记忆漏改。
- 增加针对 `failure_class=invalid_capability_id` 的 parent fallback gate 断言。
- 写窄 OpenSpec change,归档后再跑完整验证并提交。

状态:
- **目前在阶段3** - 从半成品结构字段继续完成实现与门禁。
