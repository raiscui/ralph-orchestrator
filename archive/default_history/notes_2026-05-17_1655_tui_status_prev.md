...
## [2026-05-17 16:16:00] [Session ID: omx-1779004640353-blcixq] 笔记: TUI 与 Codex/CLI 直接输出差异排查
...
- 文件: `EXPERIENCE.md`
- 要点:
  - `exp-20260430-tui-mode-before-render-bug` 明确提醒: TUI 某块“不见了”时,先确认当前 run mode。Chat / Gates 是 parallel Supervisor TUI 控制面,serial TUI 看不到它不等于渲染回归。
  - 这次排查因此优先拆分 serial TUI、parallel TUI、CLI/log-mode、record-session/agents snapshot 四条链路。

### 来源2: 并行 runner 输出与事件转发

- 文件: `crates/ralph-cli/src/parallel_runner.rs`
- 静态证据:
  - `should_forward_event_to_tui` 只转发 `gate.*`、`human.message`、`reply.human.message`、带 `source_instance` 或带 `source` 的事件。
  - TUI 模式 observer 会把 stdout/stderr 作为 `ParallelOutputChunk` 推给 UI,但 `--hide-stderr` 会阻止 stderr 进入 TUI 显示; record-session 仍会记录 stderr。
  - CLI/log-mode 会直接打印 `[instance:out|err:job=N] line`、`[instance:state] state`、初始 instances 和最终 final states。
  - event parsing 明确是 stdout-only; stderr 只作为可观测输出和诊断证据,不参与路由。
- 动态证据:
...
- 排查纠正:
  - 一开始误用 `cargo test --package ralph-cli --lib ... -- --exact`,结果是 0 tests。这不能当证据。已改用 `--bin ralph` 重新验证。

### 来源3: 并行 TUI state / panel 模型

- 文件: `crates/ralph-tui/src/state/parallel.rs`, `crates/ralph-tui/src/widgets/*.rs`, `crates/ralph-tui/src/app.rs`
- 静态证据:
  - TUI 并行模式的主视图是 `Instances`、单个选中实例的 `Output`、底部 `Chat / Gates`。
  - `InstancesPane` 只显示 instance id、state、last output age。它不显示 `last_input.preview`、当前 job id、session strategy、turn action、当前输入 topic 等。
  - `Output` 只显示“当前选中实例”的“当前 job”。历史 job 需要切换 job; 其他实例输出不会同屏展开。
  - `Chat / Gates` 会显示发送状态、gate 列表/详情,但 `reply.human.message` 的正文仍预期在 Output 里看,Chat 区域只给 `reply @source (见 Output)` 这类提示。
  - footer 目前主要是 total elapsed + active/done,没有并行实例的 last input、当前 running job、最近 event topic 详情。

### 来源4: Pretty/Console/TUI 文本渲染差异

- 文件: `crates/ralph-adapters/src/stream_handler.rs`
- 静态证据:
  - `Rendered` 模式会渲染 Markdown,并会把 `reply.human.message` 的 `<event ...>` 外壳隐藏成 payload 展示。
  - `Plain` 模式会保留 Markdown 控制符和 `<event ...>` 外壳,用于排障与复制粘贴。
  - `TuiStreamHandler` 和 `PrettyStreamHandler` 对 `reply.human.message` 的 rendered/plain 规则已有对齐测试。
- 动态证据:
...
### 来源5: agents snapshot 状态面

- 文件: `crates/ralph-core/src/agents_snapshot.rs`, `crates/ralph-cli/src/display.rs`, 当前 `.ralph/agents.json`
- 静态证据:
  - `.ralph/agents.json` 的设计目标是回答“当前有哪些实例,它们在做什么”。字段包括 `instance_id`、`hat_id`、`state`、`is_dynamic`、`last_input`。
  - `ralph agents` 表格会显示 `Last Input`。
- 当前现场样本:
  - 当前 `.ralph/agents.json` 里 `ralph#1` 是 `idle`,并带有最近 `human.message` preview: `相 对 于 当 前 项 目 的 ralph...`。
  - 这类 `last_input.preview` 信息当前没有直接出现在并行 TUI 的 Instances 行里。

## 综合发现

### 已观察到的现象

- TUI 不是 Codex/CLI stdout 的等价镜像,而是一个筛选后的观察面板。
- CLI/log-mode 更像“审计流”: 会保留 instance/job 前缀、state 行、final states、stdout/stderr 标签。
- TUI 更像“操作面”: 保留实例列表、选中实例输出、chat/gate 控件和少量状态提示。

### 当前已验证结论

- 并行 TUI 会过滤一部分不带 source/source_instance 的业务事件。这不是偶发丢失,而是 `should_forward_event_to_tui` 的明确策略。
- stderr 不参与事件解析是已验证不变量; 但 stderr 是否进入 TUI 可见面,还取决于 `show_stderr` / `--hide-stderr`。
- `Rendered` 模式不会原样显示 `reply.human.message` 的 `<event ...>` 外壳,只显示 payload。这属于展示层改写,不是语义证据丢失。
- `agents.json` 和 `ralph agents` 已经有“当前状态/最近输入”的信息,但并行 TUI 没有把这部分完整合进主画面。

### 主要缺口清单

1. **全量审计流缺口**: TUI 默认不等价展示 CLI/log-mode 的 `[instance:out|err:job=N]`、`[instance:state]`、final states 等原始审计文本。
2. **事件可见性缺口**: 无 source/source_instance 的业务事件不会转发给 TUI,除非它是 gate/human/reply 控制面事件。
3. **当前状态缺口**: TUI Instances 只显示生命周期 state + output age,没有 `last_input.preview`、当前 job id、最近 input topic、session strategy / turn action。
4. **多实例并发可见性缺口**: Output 只显示选中实例的当前 job,所以“别的实例正在输出什么”不在同屏展开。
5. **Rendered/Plain 语义差异**: Rendered 更适合阅读,但会隐藏 Markdown 控制符和部分 event wrapper。Plain 更接近原始输出,更适合排障。
6. **证据入口分裂**: record-session、events.jsonl、agents.json、evidence-index 都保留了部分真相,但 TUI 主画面没有一个“evidence/audit pane”把它们串起来。

### 推荐改良方向

- 最佳方案: 新增/改良 TUI 的“状态 + 审计”双层可视化,把当前实例状态、last input、当前 job、stderr 可见性、event audit、record-session/evidence 入口统一展示。TUI 仍保留可读模式,但提供一键切换 raw/audit 视图。
- 先能用方案: 在现有并行 TUI 上做增量增强:
  1. Instances 行增加 `last_input.preview` 或当前 input topic。
  2. Header/Footer 增加 selected instance、state、job、last event topic、stderr hidden/visible 标识。
  3. Output title 增加 stdout/stderr 显示状态和 buffer cap。
  4. 新增快捷键切换 `Rendered` / `Plain` 或 raw event wrapper 可见性。
  5. 在 Chat / Gates status 区明确提示“完整证据路径: record-session/events/agents”。



## [2026-05-17 16:24:01] [Session ID: omx-1779004640353-blcixq] 笔记: TUI 差异排查阶段4复核

## 来源

### 来源1: 当前会话 focused tests
...
- 命令与结果:
  ...
  - `cargo test --package ralph-adapters --lib stream_handler::tests::tui_stream_handler::pretty_plain_mode_keeps_reply_human_message_event_text -- --exact`: 1 passed。
- 补充证据:
  - 本轮误触发了一次全量 `cargo test`,最终退出码为 0,未观察到 error 输出。该结果只能作为额外环境健康信号,不是本轮设计判断的主要证据。

### 来源2: 关键代码复核

- `crates/ralph-cli/src/parallel_runner.rs`:
  - `should_forward_event_to_tui` 只允许 gate/human/reply 事件,以及带 `source_instance` 或 `source` 的事件进入 TUI。
  - TUI observer 默认显示 stderr,但 `--hide-stderr` 会阻止 stderr 进入 TUI 显示; cassette 仍会记录 stderr。
  - 非 TUI log-mode 结束时会打印 `[supervisor] final states:`。
- `crates/ralph-tui/src/widgets/instances.rs`:
  - Instances 当前只渲染 instance id、state、last output age。
  - 没有渲染 last input topic/preview、current job id、session strategy、last event topic。
- `crates/ralph-cli/src/display.rs`:
  - `ralph agents` 已有 `Last Input` 列,说明状态真相源不是没有,而是没有完整接入 TUI 主画面。
- `crates/ralph-adapters/src/stream_handler.rs`:
  - Rendered/Plain 的语义是明确区分的。
  - Rendered 会对 `reply.human.message` 的 event wrapper 做展示层改写,Plain 则保留原始 event 文本。

## 综合发现

### 已验证结论

- 当前 TUI 的主要问题不是 runtime 一定丢了信息,而是展示模型比 Codex/CLI 直出更窄。
- 直出/日志模式偏审计流,会把 instance/job/state/final states 等信息直接打出来。
- TUI 偏操作面,它展示当前选中实例、部分事件、gate/chat 状态,但没有把所有审计字段同屏展现。
- 如果目标是“不遗漏信息”,推荐不要把 TUI 改成无差别 stdout 镜像,而是增加 raw/audit 可见层和 richer status 摘要。

### 推荐收口

- 先能用方案: 在现有 Instances/Header/Footer/Output title 中补状态字段和 mode 标识。
- 最佳方案: TUI 保持可读操作面,同时增加 Raw/Audit 视图,明确哪些 stdout/stderr/event/cassette/evidence 当前可见或被隐藏。


## [2026-05-17 16:29:49] [Session ID: omx-1779004640353-blcixq] 笔记: hook 后追加的新鲜验证证据

## 来源

### 来源1: agents 状态面集成测试

- 命令: cargo test --package ralph-cli --test integration_agents test_agents_command_prints_table -- --exact
- 结果: 1 passed。
- 说明:
  - 该测试构造了带 last_input 的 agents snapshot。
  - 断言 ralph agents 表格会显示 writer#1、writer、running 和 build.task。
  - 这支持当前结论: last input 这类状态信息已经存在于 agents 状态面,但还没有完整接入并行 TUI 主画面。

### 来源2: 并行 TUI full layout snapshot smoke

- 命令: cargo test --package ralph-tui --test integration_snapshots test_parallel_full_layout_renders_instances_output_and_gates -- --exact
- 结果: 1 passed。
- 说明:
  - 该测试注册 writer#1,注入 stdout 输出和 gate.request,再渲染并行 TUI full layout。
  - 这支持当前结论: TUI 确实有 instances / output / gates 三块操作面,但这个测试不覆盖 last input、current job、last event 等更丰富状态字段。

## 综合发现

- 新证据没有推翻原结论。
- 当前最准确的判断仍是: 信息真相源分散存在于 output/event/agents snapshot/cassette 中,TUI 主画面没有完整聚合。
- 因此改良重点仍应是“状态摘要 + raw/audit 可见层”,而不是把 TUI 简单改成 stdout 镜像。


## [2026-05-17 16:51:58] [Session ID: omx-1779004640353-blcixq] 笔记: TUI 状态增强实现结论
...
- `crates/ralph-tui/src/state/parallel.rs`
  - 增加 `current_job_summary()` 和 `current_job_short_summary()`。
  - 目的: 复用 `InstanceViewState.jobs/current_job`,让展示层能表达当前 job 位置,不新增第二套状态源。
- `crates/ralph-tui/src/widgets/instances.rs`
  - Instances 行从 `id state age` 扩展为 `id state job x/y age`。
  - 目的: 左侧实例列表直接显示每个实例当前跑到第几个 job。
- `crates/ralph-tui/src/widgets/footer.rs`
  - 并行模式 footer 增加 selected instance、紧凑 job、render mode、last event 摘要。
  - 80 列下最终采用紧凑格式,例如 `writer#1 j1/1 m:P e:reply.human.message`。
...
- `cargo test --package ralph-tui`: 214 unit tests + 26 snapshots + 4 iteration boundary tests + doctests passed。
- `cargo test`: passed。
- `git diff --check`: passed。

## 综合发现

- 本次实现只改展示层,没有改事件路由、事件解析、stdout/stderr 语义或 runtime 调度。
- 状态真相源仍然是 `ParallelTuiState` / `InstanceViewState` / `TuiState.last_event`。
- footer 在 80 列下不能承载长文本。失败测试推动实现从 verbose label 收敛到紧凑 label。
- 这次只落地“状态摘要”层,还没落地 raw/audit 视图和 evidence path 面板。
