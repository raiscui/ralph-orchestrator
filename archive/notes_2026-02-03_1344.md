...
### 现象
- Radar 面板距离终端“右上角”有 2 行空隙。
- Zoom 后面板尺寸不跟随“字符图本身尺寸”：
  - 有时宽/高不够，字符图被裁切。
  - 有时面板过大，字符图四周留白很多。

### 根因定位
- `crates/ralph-tui/src/app.rs`：
  - 整体布局固定：Header 高度=2（内容 1 行 + bottom border 1 行），Content 从 y=2 开始。
  - Radar 之前用 `content_area` 作为锚点 bounds，导致其 y=2（视觉上“离右上角空 2 行”）。
  - Radar 之前的尺寸策略：
    - mini：固定上限（<=36x10）
    - zoom：按 content_area 的 2/3 + 上限裁剪（<=120x40）
    - 两者都未根据字符图的实际宽高做自适配。

### 修复策略
- 锚点改为“整屏去掉 footer”：
  - bounds = `frame_area`，但高度截止到 footer 起始行（避免覆盖 footer）。
  - 这样 Radar 的 y=0，真正贴到右上角。
- 新增字符图尺寸测量 `measure_text_diagram_size`：
  - 宽度：每行的 Unicode display width（使用 `unicode_width::UnicodeWidthStr`，避免 emoji/东亚字符宽度误判）。
  - 高度：行数。
- `hat_graph_radar_area` 尺寸策略调整：
  - zoom：优先适配 `diagram_size + border`，再按 bounds 裁剪（尽量完整显示/减少留白）。
  - mini：仍保留“雷达感”上限（<=36x10），但当字符图更小时会收缩，减少无意义留白。

### 相关落点
- `crates/ralph-tui/src/app.rs`：锚点 bounds + 自适配算法 + 回归测试
- `specs/terminal-ui.spec.md`：更新 Radar 的锚点语义与 zoom 自适配说明

---
## 2026-02-03 13:16 +0800｜更正：Radar 需要向内偏移（往左下移）
- 需求澄清：Radar 不是要贴边，而是要从右上角向内留白（偏移到左下）。
- 实现：`hat_graph_radar_area` 增加 `HAT_GRAPH_RADAR_INSET_X/Y = 2`，并裁剪可用 bounds。
- 同步：更新 `specs/terminal-ui.spec.md` + 回归测试（期望 y=inset_y，而非 y=0）。
