...
## 2026-03-02 16:22 +0800 | feat/fix: emit-control-plane-fail-closed(OpenSpec apply 17/17)

- 完成 OpenSpec change `emit-control-plane-fail-closed` 的全部任务(17/17),并已勾选 `openspec/changes/emit-control-plane-fail-closed/tasks.md`。

### 代码改动

1. `crates/ralph-cli/src/main.rs`
- 新增 `validate_emit_control_plane_args()`。
- `emit_command()` 写入 JSONL 前执行 fail-closed 校验。
- 新增 CLI 回归测试:
  - hat 环境拒绝 `--turn-action steer|interrupt`
  - 缺失 `target_instance` 拒绝
  - 非 `ralph#1` 拒绝
  - 与 `--target`/`--spawn-instance` 组合拒绝

2. `crates/ralph-core/src/parallel/supervisor/routing.rs`
- 新增 external control-plane 校验辅助逻辑:
  - `is_external_jsonl_event()`
  - `validate_external_control_plane_turn_action()`
  - `reject_external_control_plane_turn_action()`
- 对 external `turn_action=steer|interrupt` 执行硬拒绝规则并输出 warning。
- 拒绝时复用 `routing.escalate` 告警到 `ralph#1`。

3. `crates/ralph-core/src/parallel/supervisor/routing_tests.rs`
- 新增回归测试:
  - `external_turn_action_missing_target_instance_is_rejected_and_escalated`
  - `external_turn_action_non_ralph_target_is_rejected_and_escalated`
  - `external_turn_action_with_target_or_spawn_hint_is_rejected_and_escalated`

4. `crates/ralph-tui/src/app.rs`
- 新增 `validate_human_message_turn_action_target()`。
- chat 提交 `HumanMessage` 时,非 `ralph#1` 的 steer/interrupt 在本地直接拒绝,不写 JSONL。
- 新增对应单测 3 条。

### 文档/规范同步

- `specs/parallel-event-channels.spec.md`
  - 增补 external control-plane fail-closed 边界(S5.1)。
  - 增补 hat-to-hat request/result 最终结论约束(S5.2)。
- `examples/parallel-experimental-dev-engine/README.md`
  - 明确 TUI/CLI steer/interrupt 只能作用于 `ralph#1`。
- `config/all_hat.md`
  - 移除误导性 worker steer 示例,改为 `ralph#1`。
...
## 2026-03-02 16:40 +0800 | e2e 实跑: 干净目录验证 + 官方 ralph-e2e 场景
...
- 使用二进制:
  - 当前工作树编译产物 `target/debug/ralph`（通过 `cargo run --manifest-path .../Cargo.toml --bin ralph ...` 调用）

#### 验证结果

- CLI fail-closed 有效:
  - 非 `--target-instance ralph#1` + `--turn-action` 被拒绝。
  - hat 环境(`RALPH_HAT_INSTANCE_ID`)下使用 `--turn-action` 被拒绝。
- Supervisor fail-closed 有效:
  - external JSONL 非法 `turn_action` 事件被拒绝。
  - stderr 出现 `Rejected external control-plane turn_action event` warning。
  - `.ralph/events.jsonl` 出现 3 条 `routing.escalate` 给 `ralph#1` 的记录,覆盖:
    - 非 `ralph#1` target_instance
    - `target` hint + turn_action
...
## 2026-03-02 15:22 +0800 | e2e: live steer 场景通过(parallel-app-server-steer-live-reply-multi-turn)

- 触发背景:
  - 用户在后续建议中选择 `1`,要求继续执行 live steer 场景验证。
- 执行命令:
...
- 结论:
  - 你选择的后续项 `1` 已完成。
  - 当前改动在该 live steer e2e 场景下验证通过。

## 2026-03-02 16:12 +0800 | fix(e2e): 默认 markdown 同步刷新 report.json

- 问题:
  - `ralph-e2e` 默认 `--report markdown` 只更新 `report.md`,可能遗留旧 `report.json`。
- 根修:
  - 修改 `crates/ralph-e2e/src/main.rs` 的 CLI 报告格式映射:
    - `ReportFormat::Markdown` -> `LibReportFormat::Both`
  - 这样默认路径会同步写入 md + json,保证同一轮结果一致。
- 测试补充:
  - 在 `crates/ralph-e2e/src/main.rs` 新增 3 条单测,锁定格式映射语义。
- 文档同步:
  - 更新 `crates/ralph-e2e/README.md` 的 Report Formats 描述,明确默认会刷新 JSON 快照。
- 验证结果:
  - `cargo test -p ralph-e2e` ✅
  - `cargo test` ✅
  - `cargo run -p ralph-e2e -- codex --mock --filter parallel-emit-spawn-instance --report markdown --skip-analysis` ✅
  - 运行输出出现两条写盘日志(`report.md` + `report.json`),并且两者 mtime 一致。

## 2026-03-02 16:39 +0800 | verify(e2e-live): 默认 markdown 在真实 backend 同步刷新 report.json

- 背景:
  - 用户选择继续执行 live 复验,确认修复不只在 mock 生效。
- 执行:
...
- 结果:
  - PASS,90.1s。
- 一致性证据:
  - 终端输出同时写入 `report.md` 与 `report.json`。
  - `report.md` 与 `report.json` 场景/通过数/耗时一致。
  - 两文件 mtime 同秒(`16:38:06`)。
- 结论:
  - 默认 `--report markdown` 在 live backend 也稳定同步 json 报告。

## 2026-03-03 10:29 +0800 | improve(record-session): 追加 session 元信息 + 关键记录立即 flush

- 背景/动机:
  - 你反馈 `--record-session` 的 JSONL 里需要包含基本信息(项目路径/运行命令)。
  - 同时你在 `ralph-example` 目录里只问了 1 条 `human.message`,record 看起来“没回复就结束”。
  - 复现后确认: recorder 使用 `BufWriter` 时,在运行中或中断场景下,尾部记录可能还在缓冲区,从磁盘读会产生错觉。

- 实现:
  - `crates/ralph-core/src/session_recorder.rs`:
    - 新增 `_meta.session_start`(cwd/workspace_root/argv/pid)。
    - 对关键记录做立即 flush:
      - `_meta.*`
      - `bus.publish`
      - `ux.terminal.write` 的 stdout
  - `crates/ralph-cli/src/loop_runner.rs`:
    - 启用 `--record-session` 时写入 `_meta.session_start`。
  - `crates/ralph-cli/src/parallel_runner.rs`:
    - 启用 `--record-session` 时写入 `_meta.session_start`。
  - `specs/parallel-record-session.spec.md`:
    - spec 同步: 追加 `_meta.session_start` 与关键 flush 约束。

- 验证:
  - 手工复跑: 并行 idle_start + human.message -> reply.human.message:
    - 在进程未退出时读取 JSONL,已能看到 `reply.human.message` 相关 stdout 与 bus.publish 证据。
  - 自动化:
    - `cargo fmt` ✅
    - `cargo test -p ralph-core` ✅
    - `cargo test -p ralph-cli` ✅
...
## 2026-03-03 10:55 +0800 | follow-up: 新增注释统一半角标点 + 复验

- 动机:
  - 本次新增中文注释里混入了少量全角引号/句号,会降低可读性与一致性.
- 调整:
  - 仅对本次新增注释做半角标点统一(不改动历史注释).
- 复验:
  - `cargo fmt` ✅
  - `cargo test -p ralph-core` ✅
  - `cargo test -p ralph-cli` ✅
