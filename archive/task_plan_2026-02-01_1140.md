...
# 任务计划：并行 TUI 在 `LOOP_COMPLETE` 后“暂停不退出”+ 禁用实例回收

## 目标

- 当启用 TUI（parallel-tui）时：
  - `ralph#1` 输出 `LOOP_COMPLETE` 之后，Supervisor **不退出**。
  - 运行时进入“暂停/停歇”状态：不再因为历史 job 的“延迟事件”继续派生新 job。
  - 但仍继续监听外部事件（`human.message` / `gate.resolve` 等），收到新输入后可恢复继续对话与工作。
- 当未启用 TUI（parallel-cli / CI / E2E）时：
  - 行为保持不变：`LOOP_COMPLETE` 仍然是“自然结束信号”，并按现有 drain 逻辑退出。
- 并行 TUI 模式下：实例**不回收**
  - 重点：禁用 autoscale 动态实例的 idle TTL 回收（避免进入 `done` 后无法继续对话）。

## 方案（至少二选一）

### 方案 A：只对 TUI 生效（你已选择）

- `LOOP_COMPLETE` 语义：从“退出”改为“暂停等待 human 输入”。
- 禁用动态实例 idle 回收，实例保持可继续接收事件/继续跑 job。

### 方案 B：保持现状（不改语义）

- 继续把 `LOOP_COMPLETE` 视为“终止信号”，Supervisor 自然退出。
- 需要继续对话只能开启新 run（新的 `.ralph/current-events` / scratchpad），对你来说不满足。

> 你已明确选择：方案 A（只对 TUI 生效 + 实例不回收）。

## 阶段

- [x] 阶段1：写 spec（specs/）并确认行为边界
- [x] 阶段2：实现 Supervisor “暂停态”（保留收敛护栏，不退出）
- [x] 阶段3：禁用动态实例 idle 回收（仅 TUI）
- [x] 阶段4：补测试（防回归）+ 全量验证（含 smoke tests）
- [x] 阶段5：四文件记录与收尾建议

## 关键问题

1. “暂停态”在并行运行时如何定义？
   - 必须继续消费外部事件（human.message）。
   - 必须继续保持“completion 收敛护栏”：避免 completion 后仍继续路由历史事件派生新 job。
2. “实例不回收”范围？
   - 按你的要求：至少禁用 **动态实例** idle TTL 回收（避免 done）。
   - 静态实例本来就不会自动回收（仅在退出/中断时 shutdown）。

## 做出的决定

- [x] 只对 TUI 生效：completion promise 进入暂停态，不退出
- [x] 并行 TUI 模式禁用实例回收（动态 idle TTL 回收关闭）

## 状态

**已完成**
- 并行 TUI 下 `LOOP_COMPLETE` 已改为“暂停不退出”，并且已禁用动态实例 idle 回收。
- 已补测试并完成全量验证（fmt/clippy/test/smoke）。

## 日志

### 2026-02-01

- [启动] 需求：并行 TUI 在 `LOOP_COMPLETE` 后暂停不退出，仍可继续对话；且实例不回收。
- [决策] 采用方案 A：仅 TUI 生效 + 禁用动态实例 idle 回收。

### 2026-02-01 11:34 +0800

- [spec] 已补充/同步 `specs/parallel-hat-instances.spec.md`：明确 TUI 下 completion promise=暂停语义 + 禁用动态实例回收。
- [impl] 已实现 completion “暂停态”：
  - 保留 completion 收敛护栏（暂停态不路由内部延迟事件派生新 job）
  - 仍持续消费 external events（human.message 等），收到外部事件后解除暂停并恢复正常路由
- [impl] 已在 parallel-tui 下禁用动态实例 idle TTL 回收（避免 instance 进入 `done` 断对话）。
- [test] 已补充回归测试，并完成 `cargo fmt` / `cargo clippy -D warnings` / `cargo test` / smoke tests 验证。
...
# 任务计划：Markdown 配色微调（inline code 取消背景）

## 目标

- inline code（行内代码）取消背景色（不再单独铺底）。
  - 目的：减少“黑块/灰块”的视觉干扰，并与 code block 的“无背景”策略一致。
- Markdown 的“红色系”统一改为 `#ff6188`（你指定的颜色）。
  - 说明：先把 inline code（raw/inline 语义）与 bold 的强调色同步到该值，后续如需细分再拆分。
- 保证 `cargo fmt --check`、`cargo clippy --all-targets --all-features -- -D warnings`、`cargo test`、replay smoke tests 通过。

## 阶段

- [x] 阶段1：调整 `default_markdown_skin()`（inline code bg → none；markdown red → #ff6188）
- [x] 阶段2：更新回归测试（锁定 inline code bg 为 None；red 为 #ff6188）
...
**已完成**：
inline code 已取消背景色，并将 Markdown 红色系统一改为 `#ff6188`（stdout + TUI 同步生效）。
同时更新了回归测试锁定关键配置，并完成全量验证（fmt/clippy/test + replay smoke tests）。

## 日志

### 2026-02-01 11:19 +0800

- [启动] 你提出微调：inline code 同样取消背景色。
- [补充] 你又提出：Markdown 红色改为 `#ff6188`。

### 2026-02-01 11:23 +0800

- [完成] inline code 取消背景色（不再铺底），并将 inline code / bold 的红色系改为 `#ff6188`。
- [测试] 更新回归测试：锁定 inline code 的 fg 为 `#ff6188`，bg 为 `None`。
- [验证] 通过：
  - `cargo fmt --check`
  - `cargo clippy --all-targets --all-features -- -D warnings`
  - `cargo test`
...
# 任务计划：Markdown 配色微调（取消 code block 背景 + 标题改为 #ffd866）

## 目标

- fenced code block（代码块）取消背景色（不再单独铺底）。
  - 目的：减少大段输出时的“背景色块”视觉噪音，避免与面板底色冲突。
- 标题统一改为 `#ffd866`（你指定的黄色）。
  - 目的：标题更亮更“Monokai”，同时减少橙色标题的刺激感。
- 保证 `cargo fmt --check`、`cargo clippy --all-targets --all-features -- -D warnings`、`cargo test`、replay smoke tests 通过。

## 阶段

- [x] 阶段1：调整 `default_markdown_skin()`（code block bg → none，heading fg → #ffd866）
- [x] 阶段2：更新/补回归测试（锁定 code block bg 为 None、heading fg 为 #ffd866）
- [x] 阶段3：验证（fmt/clippy/test/smoke）
- [x] 阶段4：四文件记录（notes/WORKLOG）
...
**已完成**：
已取消 fenced code block 的背景色，并将标题颜色统一改为 `#ffd866`（stdout + TUI 同步生效）。
同时更新了回归测试锁定关键配置，并完成全量验证（fmt/clippy/test + replay smoke tests）。

## 日志

### 2026-02-01 01:07 +0800

- [启动] 你提出微调：
  - 代码块取消背景色
  - 标题颜色改为 `#ffd866`

### 2026-02-01 01:10 +0800

- [完成] fenced code block 取消背景色（不再铺底），标题统一使用 `#ffd866`。
- [测试] 新增/更新回归测试：锁定 code block bg 为 `None`、heading fg 为 `#ffd866`。
- [验证] 通过：
  - `cargo fmt --check`
  - `cargo clippy --all-targets --all-features -- -D warnings`
  - `cargo test`
...
# 任务计划：Markdown 内部配色切换为 Monokai Pro

## 目标

- `termimad` 渲染的 Markdown（stdout + TUI）使用 Monokai Pro 调色板（你提供的 13 个颜色变量）。
- 保持 Markdown 语义不变（粗体/斜体/标题/代码/引用/列表仍按原逻辑渲染），只调整颜色与少量可读性属性。
- 保证 `cargo fmt --check`、`cargo clippy --all-targets --all-features -- -D warnings`、`cargo test`、replay smoke tests 通过。

## 方案（至少二选一）

### 方案 A：仅影响 Markdown skin（推荐，我将采用）

- 只改 `crates/ralph-adapters/src/stream_handler.rs` 中的 `default_markdown_skin()`。
- stdout 与 TUI 渲染统一生效（两条路径复用同一个 `MadSkin`）。
- 不碰整体 TUI 的主题色（header/footer/border 等），避免范围扩张。

### 方案 B：同时同步到文档站点（MkDocs Material）

- 除了 termimad skin，也在 `docs/overrides/` 增加 CSS，覆盖 docs 站点的代码块/inline code 高亮到 Monokai Pro。
- 需要你确认是否希望 docs 网站也跟着变化（否则属于范围扩张）。

## 阶段

- [x] 阶段1：确认改动范围与颜色映射
- [x] 阶段2：实现 Monokai Pro `MadSkin`
- [x] 阶段3：补回归测试（锁定关键颜色）
- [x] 阶段4：验证（fmt/clippy/test/smoke）
- [x] 阶段5：四文件记录（notes/WORKLOG）

## 关键问题

1. `accent1~6` 的映射我先按“标题彩虹阶梯”处理：H1→accent6，H2→accent5，…，H6→accent1。
2. `background/dark1/dark2` 我先用于 code block / inline code 的背景，正文不强制设置背景色，避免与用户终端主题冲突。

## 状态

**已完成**：
Markdown 渲染已切换到 Monokai Pro 配色（stdout + TUI 同步生效）。
同时新增回归测试锁定关键颜色映射，避免未来回退到默认灰度。
全量验证（fmt/clippy/test/smoke）已通过。

## 日志

### 2026-02-01 00:16 +0800

- [启动] 新需求：你希望 Markdown 的内部配色使用 Monokai Pro 调色板。
- [计划] 先定位 `termimad::MadSkin` 的可配置项，再把 palette 映射进 `default_markdown_skin()`，最后用测试 + 全量验证做背压。

### 2026-02-01 00:34 +0800

- [完成] 在 `default_markdown_skin()` 中落地 Monokai Pro 调色板映射（正文/标题/inline code/code block/引用/列表/表格等）。
- [处理] 为避免 workspace `crossterm`（0.28）与 `termimad` 依赖的 `crossterm`（0.29）发生类型不匹配，palette 常量使用 `termimad::crossterm::style::Color`。
- [测试] 新增回归测试：
  - `markdown_inline_code_uses_monokai_pro_palette`
  - `markdown_fenced_code_block_uses_monokai_pro_palette`
  - 注：测试直接断言 `MadSkin` 内部颜色配置，避免 `NO_COLOR=1` 导致 ANSI 输出被抑制而误判。
- [验证] 通过：
  - `cargo fmt --check`
  - `cargo clippy --all-targets --all-features -- -D warnings`
  - `cargo test`
...
# 任务计划：Markdown 内部配色改为 sublime-monokai-extended

## 目标

- 将 `termimad` 渲染的 Markdown（stdout + TUI）配色切换为：
  - `jonschlinkert/sublime-monokai-extended` 的 **Monokai Extended** 主题风格。
- 优先遵循该主题对 Markdown 的 scope 配色（heading / quote / bold / italic / raw 等），同时结合终端阅读体验做最少量调整（比如 code block 背景）。
- 保证 `cargo fmt --check`、`cargo clippy --all-targets --all-features -- -D warnings`、`cargo test`、replay smoke tests 通过。

## 方案（至少二选一）

### 方案 A：只影响 Markdown skin（推荐，我将采用）

- 只改 `crates/ralph-adapters/src/stream_handler.rs` 的 `default_markdown_skin()` 与 palette 常量。
- stdout 与 TUI 两条渲染路径自然同步（复用同一个 `MadSkin`）。
- 不扩张到全局 TUI 主题（边框/面板/标题栏），避免范围失控。

### 方案 B：同时同步到文档站点（MkDocs Material）

- 在 `docs/overrides/` 加 CSS，把 docs 站点的 code block/inline code 也同步到 Monokai Extended。
- 需要你确认是否希望 docs 网站也跟着改（否则属于范围扩张）。

## 阶段

- [x] 阶段1：提取主题关键色 + 映射到 termimad `MadSkin`
- [x] 阶段2：更新回归测试期望值（锁定关键色）
- [x] 阶段3：验证（fmt/clippy/test/smoke）
- [x] 阶段4：四文件记录（notes/WORKLOG）

## 关键问题

1. 主题 repo 里有多个变体（Bright/Light 等）。我会先按默认的 **Monokai Extended（暗色）** 落地。

## 状态

**已完成**：
已将 Markdown 内部配色切换为 `sublime-monokai-extended` 的 Monokai Extended 风格（stdout + TUI 同步生效）。
同时更新回归测试锁定关键色，避免未来回退到旧 palette。
全量验证（fmt/clippy/test + replay smoke tests）已通过。

## 日志

### 2026-02-01 00:55 +0800

- [启动] 你反馈：Monokai Pro 的配色不好看，希望改用 `sublime-monokai-extended` 的配色。
- [计划] 我将按“方案 A”执行：只改 `termimad` 的 Markdown skin，并跑全量验证与 replay smoke tests 做背压。

### 2026-02-01 00:59 +0800

- [完成] `default_markdown_skin()` 已切换到 `sublime-monokai-extended`（Monokai Extended.tmTheme）配色：
  - heading / quote / bold / italic / strike / inline code / code block / list 等关键元素已映射。
- [测试] 已更新并新增回归测试期望值，锁定 inline code 与 code block 的主题色配置（避免受 `NO_COLOR=1` 影响）。
- [验证] 通过：
  - `cargo fmt --check`
  - `cargo clippy --all-targets --all-features -- -D warnings`
  - `cargo test`
