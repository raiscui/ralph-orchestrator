...
## 2026-01-29 20:08 +0800｜TUI chat `Shift+Enter` 兼容性与换行 fallback

### 现象
- 在并行 Supervisor TUI 的 chat 输入框里，用户反馈 `Shift+Enter` 无法换行，仍会触发“提交/发送”。

### 本质（根因）
- `crossterm::event::KeyModifiers` 确实提供了 `SHIFT/ALT/CONTROL` 的 bitflags，并且可以用 `key_event.modifiers.contains(KeyModifiers::SHIFT)` 判断修饰键。  
- 但终端是否会把 `Shift+Enter` 与普通 `Enter` 区分并上报给应用，是**终端实现相关**的：部分终端对 Enter 不携带 SHIFT 修饰符，导致 TUI 侧看起来就是一次普通 Enter。

### 修复方向（可落地）
- 保留 `Shift+Enter` → newline 的逻辑（对“确实能区分”的终端仍然生效）。
- 增加更稳定的换行 fallback：
  - `Alt+Enter`（容易被终端区分，crossterm 可通过 `KeyModifiers::ALT` 检测）
  - `Ctrl+J`（很多终端/TTY 习惯把它当作 linefeed，可通过 `KeyCode::Char('j') + KeyModifiers::CONTROL` 检测）
- 同步更新 TUI 状态栏与 help overlay，让用户不需要猜键位。

---

## 2026-01-29 20:35 +0800｜TUI 框选后 Cmd+C/Cmd+V 不工作：根因与修复策略

### 现象
- 在 TUI 输出面板里用鼠标框选（蓝色高亮）后，按 `Command+C` / `Command+V` 无法完成复制粘贴闭环。

### 根因（结合代码直接引用）
1) TUI 启动时开启了 mouse capture，会把鼠标事件交给应用处理，终端模拟器的“原生文本选择”通常就不生效了：

> `execute!(stdout, EnterAlternateScreen, EnableMouseCapture)?;`

2) 终端的 `Command+C` / `Command+V` 是**终端模拟器快捷键**，默认不会以“按键事件”形式交给应用：
   - 如果应用内只是“高亮了选择区域”，但没有主动写入剪贴板，那么按 `Cmd+C` 不会得到任何东西。

3) 粘贴侧：如果终端使用 bracketed paste，crossterm 可能上报 `Event::Paste(text)`。
   - 之前代码把 Paste 事件视作“其它事件”直接忽略（用户会感知为 `Cmd+V` 没反应）。

### 修复策略（落地）
- 把“应用内选择”转换成“系统剪贴板内容”：
  - MouseUp（结束框选）自动复制（best-effort），让用户继续用 `Cmd+V` 即可粘贴。
  - 同时提供显式 `y` 复制，作为可发现的兜底。
- 增加 `Event::Paste(text)` 的处理：
  - search mode：追加到搜索输入（压平换行）
  - chat focus：插入到 chat editor（保留换行）

---

## 2026-01-29 20:28 +0800｜调研：`benjajaja/mdfried` 的 Markdown 渲染路线与许可证风险

### 结论（对本项目最关键）
- `mdfried` / `mdfrier` 的 `license = "GPL-3.0-or-later"`，而本项目为 MIT License，因此**不能直接依赖或复用其代码**（否则会引入强 copyleft 风险）。
- `mdfrier` 的定位是“markdown parser that produces styled terminal lines”，并使用 `tree-sitter-md` + `tree-sitter` 来解析 Markdown，再输出带样式的终端行（可选 `ratatui` feature）。

### 对应策略
- “学习其技术思路”而不是“引入其 crate”：我们可以用 `pulldown-cmark` / `comrak` / `tui-markdown` 等宽松许可证依赖，实现 best-effort 渲染与安全降级。

---

## 2026-01-29 21:30 +0800｜并行 TUI：Chat 输入行与 Targets 行“太挤”+ 灰色太暗

### 现象
- Chat 输入框里只输入 1 行时，文本/光标会贴着输入框底线显示，紧挨着下方 `Targets: @ralph#1 ...`，视觉上“靠得太近”。
- 多处提示/标签使用 `Color::DarkGray`，在部分终端主题下对比度太低，阅读吃力。

### 根因
- 之前为了解决“太靠上”，输入框做了“底部对齐”（上方 padding）。
  - 但在 3 行高度的输入框里，单行内容会落在最后一行，导致与下方区域没有呼吸间距。
- `DarkGray` 在某些主题里接近背景色，导致“灰色信息不可见”。

### 修复策略（本次）
- 输入框在“底部对齐”时保留 1 行底部留白，让单行输入“上抬”一行，同时同步 hit-test 逻辑保持一致。
- 将 TUI 中的 `Color::DarkGray` 统一提亮为 `Color::Gray`，优先解决可读性问题。

---

## 2026-01-29 22:05 +0800｜并行 TUI：为什么 Shift+Enter 不生效（而 Codex 可以）

### 现象
- 在当前并行 TUI 里：`Ctrl+J` 可换行，但 `Shift+Enter` 仍然表现为普通 `Enter`（提交）。

### 根因（关键差异点）
- 很多终端在默认键盘上报模式下，**不会**把 `Shift+Enter` 与 `Enter` 区分开来（应用层拿不到 `KeyModifiers::SHIFT`）。
- crossterm 提供了 kitty keyboard protocol（progressive enhancement）来解决这类“修饰键歧义”：
  - 原文（crossterm 0.28.1 注释，摘录）："adds extra information to keyboard events and removes ambiguity for modifier keys."
- Codex 能用 `Shift+Enter` 的原因，很可能就是它在启动时启用了该协议；而我们之前没有启用。

### 修复策略（落地）
- TUI 启动时 best-effort 执行：
  - `PushKeyboardEnhancementFlags(KeyboardEnhancementFlags::DISAMBIGUATE_ESCAPE_CODES)`
  - 退出时配对 `PopKeyboardEnhancementFlags`
- 这样在支持 kitty protocol 的终端里，`Shift+Enter` 会被上报为“带 SHIFT 的 Enter”，从而触发我们现有的换行分支。

---

## 2026-01-29 22:05 +0800｜灰色样式：`DarkGray` 太暗、`Gray` 太亮（接近白色）

### 现象
- 你的终端主题下 `Color::Gray`（ANSI 37）看起来接近白色，失去“提示灰”的语义。

### 修复策略（落地）
- 引入 `MUTED_FG = Color::Indexed(245)`（256 色灰阶中灰），作为 `DarkGray` 与 `Gray` 的折中值。
- 通过 `crates/ralph-tui/src/theme.rs` 集中管理，避免到处散落硬编码。
