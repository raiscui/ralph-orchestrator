...
## 2026-01-28 19:26:20 +0800｜退出 explore mode：固化 parallel 的“官方语义”（workflow entry / completion / orphan / chain topology）

### 我正在做什么 & 为什么
- 我正在把我们刚刚讨论并达成一致的“parallel 工作流语义”固化成一个新的 OpenSpec change。
- 这样做是为了：
  - 让 `event_loop.starting_event`、并行启动、完成条件、orphan 兜底等概念有**单一权威定义**，避免示例/文档/实现各说各话。
  - 后续改代码/改文档时，有清晰的验收标准与回归约束。

### 两个方向（都能落地，你已倾向方向A）
- 方向A（推荐，符合你的“链式老板兜底”哲学）：以 **chain topology** 为默认推荐拓扑，Ralph 只兜底真正 orphan，避免 100 个 hat 时“老板被所有事打断”。
- 方向B（先能用，后面再优雅）：保持现状（demo prompt 驱动 + ralph wildcard fallback），只补文档说明与示例规范，减少短期代码改动，但长期更容易继续产生困惑。

### 这次 change 的核心决议（来自你刚刚的确认）
1. 拓扑默认推荐：链式（老板/总监兜底），而不是“所有 orphan/控制面事件都汇总到 ralph#1”。
2. `starting_event` 语义：作为“协调后 workflow entry event（可由 ralph#1 决定）”，不是 runtime 的第一条事件。
3. workflow 级完成信号：新增（或明确）`complete_publishes: "<topic>"`（唯一值，定义该 ralph.yml 的最终结束信号）。
4. 完成事件如果没有订阅者：作为 orphan 交给 ralph#1，由 ralph#1 决定输出 `LOOP_COMPLETE` 或继续新一轮（你选 B）。
5. orphan 边界：只有“没有任何接受者”的事件才升级给 ralph#1；未来若有“经理 hat”接住，则不再打扰老板。
6. 控制面事件：允许统一前缀 `ralph.*`（用于内部/兜底类 topic 的命名约定）。
7. 并发上限 K：绑定 `parallel.autoscale.max_running_jobs`。

### 我接下来会做什么（本回合只做 change scaffold，不先写工件内容）
1. `openspec new change "<name>"` 创建 change 目录（默认 schema：`spec-driven`）。
2. `openspec status --change "<name>"` 确认第一个可写工件（通常是 `proposal`）。
3. `openspec instructions <artifact> --change "<name>"` 输出首个工件的模板与上下文。
4. 停止，等待你确认是否继续：我再开始写 `proposal/design/specs/tasks`。

### 执行结果（2026-01-28 19:26:20 +0800）
- [x] 已创建 change scaffold：`openspec/changes/parallel-workflow-semantics/`（schema: `spec-driven`）
- [x] 已确认工件状态：`proposal` ready；`design/specs` blocked by `proposal`；`tasks` blocked by `design,specs`
- [x] 已输出首个工件 `proposal` 的 instructions/template（下一步可直接开始写 `proposal.md`）
- [ ] 等你确认：是否继续由我起草 `proposal.md`

## 2026-01-28 19:40:32 +0800｜开始 apply：实现 parallel-workflow-semantics（按 openspec tasks.md）

### 我正在做什么 & 为什么
- 我正在进入实现阶段（apply），按 `openspec/changes/parallel-workflow-semantics/tasks.md` 的 12 条任务逐条落地。
- 这样做是为了把“官方语义”从文档/规格，真正落到代码路径与 smoke tests 上，形成可验证的闭环。

### 当前输入（本轮 apply 的上下文文件）
- `openspec/changes/parallel-workflow-semantics/proposal.md`
- `openspec/changes/parallel-workflow-semantics/specs/**/*.md`
- `openspec/changes/parallel-workflow-semantics/design.md`
- `openspec/changes/parallel-workflow-semantics/tasks.md`

### 执行策略（你之前的偏好）
- 默认推荐链式拓扑：老板（ralph#1）兜底，但不被 wildcard 经理打扰。
- completion 仍以 ralph#1 输出 `event_loop.completion_promise` 为“硬退出信号”，`event_loop.complete_publishes` 作为“完成候选事件 topic”。
...
## 2026-01-28 15:59:06 +0800｜规格对齐：parallel-trigger-routing 观点更新到 parallel-hat-instances 模型下

### 我正在做什么 & 为什么
- 我在对齐主规格里 `parallel-trigger-routing` 与 `parallel-hat-instances` 的重叠语义。
- 这样做是因为这两份 spec 是“同一并行运行时”的不同层次描述。
- 如果不对齐，会出现“是否必须配置 TopicContract / 实例选择是否允许 LLM”等表述冲突，影响后续维护与新同学理解。

### 已完成的对齐点（主规格层）
- TopicContract：统一为“总是 resolve”，未配置命中时由 triggers 派生默认 TopicContract。
- queue selection：把 trigger-routing 的 idle-first deterministic 限定为 deterministic 默认策略；`queue_selection=llm` 走落盘可回放约束。

## 2026-01-28 16:05:45 +0800｜OpenSpec：归档完成的 changes 到 openspec/changes/archive

### 我正在做什么 & 为什么
- 我正在把已经完成（complete）的 changes 归档到 `openspec/changes/archive/`。
- 这样做是为了让 `openspec/changes/` 只保留“进行中”的变更，降低误读与维护噪音。
...
### 执行步骤
1) 确认 change 状态与 tasks 勾选都为 complete
2) 确认归档目标目录不存在（避免覆盖）
3) 移动到 `openspec/changes/archive/YYYY-MM-DD-<name>/`
4) 归档后复核 `openspec list --json` 输出

## 2026-01-28 20:50:27 +0800｜/opsx:apply：parallel-workflow-semantics（并行工作流官方语义固化）

### 我正在做什么 & 为什么
- 我在把并行模式里 `starting_event` / `task.start` / `complete_publishes` / orphan 路由边界固化成“单一权威语义”。
- 这样做是为了消除“靠 demo prompt 才能闭环”的困惑。
- 同时也为未来 100+ hats 的链式拓扑做铺垫：老板（ralph#1）只兜底真 orphan 与最终收敛点。

### 本轮完成（按 change tasks.md）
- [x] 1.x：新增 `event_loop.complete_publishes`（唯一、可选）+ 校验 + 单测
- [x] 2.x：并行模式下 `task.start/task.resume` 作为控制面 topic，强制只路由到 `ralph#1`（防 prompt pollution）
- [x] 3.x：触发路由链式 fallback（specific > wildcard > orphan→ralph#1）+ 单测
- [x] 4.x：并行 `ralph#1` prompt 注入官方语义（starting_event / complete_publishes）并提供 hats 拓扑表
- [x] 5.x：修正文档图表语义 + 新增 replay smoke fixture 覆盖 completion candidate
...
### 状态
**完成：parallel-workflow-semantics tasks 全部勾选完成，可归档**

## 2026-01-28 21:56:16 +0800｜验证：E2E（parallel-hat-instances / backend=codex）通过

### 我正在做什么 & 为什么
- 我在跑并行 Tier-8 的 E2E 场景（`parallel-hat-instances`），并补一轮 replay smoke tests。
- 这样做是为了确认：并行语义固化 + CLI 并行 runner 的输出处理，没有引入“闭环断裂 / 事件统计异常”的回归。

### 结果
- E2E 报告：`.e2e-tests/report.md` 显示 `parallel-hat-instances` ✅（backend=Codex）
- 关键断言 `Parallel events recorded` 的统计为：`build.task: 3, build.done: 2, test.done: 1`

### 额外验证
- `cargo test -p ralph-core smoke_runner` ✅
- `cargo test` ✅

## 2026-01-28 23:16:30 +0800｜核对：parallel 入口/终点语义是否已写入“权威文档”+ E2E 变体鲁棒性回归

### 目标
- 解释清楚：为什么示例里用 `examples/parallel-trigger-routing/prompt.md`，但并不意味着 parallel 必须依赖 prompt 才能闭环。
- 核对并确认：`starting_event` / `complete_publishes` / orphan 边界是否已经写入项目的“权威描述”（docs/spec），避免团队口径漂移。
- 按你要求：额外跑两次 E2E（prompt 变体），验证解析/路由在“prompt 含伪事件示例/代码块”场景下的稳定性。

### 阶段
- [x] 阶段1：核对权威描述（docs/spec/notes）
- [x] 阶段2：E2E 两次变体回归（Codex，parallel-hat-instances）
- [x] 阶段3：汇总结果并写入 WORKLOG/notes

### 关键问题（待回答/核对）
1. `event_loop.starting_event` 的官方语义是否已明确：它不是第一条事件，而是协调后 workflow entry event？
2. `event_loop.complete_publishes` 的官方语义是否已明确：它是 completion candidate，最终是否结束由 `ralph#1` 决定？
3. orphan 的边界是否已明确：只有“无任何订阅者（含 wildcard manager）”才进入 `ralph#1` 兜底？
4. 示例 `prompt.md` 是否仅用于“确定性 demo”，而不是 parallel runtime 的必需品？

### 状态
**已完成阶段3**：权威语义已核对完毕，且 E2E 变体（variant1/variant2）均通过；已把结果写入 WORKLOG/notes。
