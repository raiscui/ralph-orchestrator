...
# Allow whitelisted commands during replay (only used if cassette contains bus events with commands)
ralph-e2e --mock --mock-allow "ralph task add,ralph tools memory add"
```
...
|----------|-------------|---------|
| `RALPH_MOCK_ALLOW` | `mock-cli` 的命令白名单（覆盖 `--allow`，便于 CI 注入） | None |
