...
### 验证
- `cargo test` ✅
- 2026-01-29 19:54: 启动 `/opsx:new` 流程，准备新建一个 OpenSpec change（当前先等待用户给出 change 内容与名称）。
- 2026-01-29 19:58: 创建 OpenSpec change `tui-markdown-rendering`（schema: `spec-driven`），并输出首个 artifact `proposal` 的 instructions/模板（未创建任何 artifacts 文件）。
- 2026-01-29 20:05: 已写入 `openspec/changes/tui-markdown-rendering/proposal.md`（默认渲染 Markdown，`--plain` 关闭），并输出下一步 artifact `design` 的 instructions/模板。
