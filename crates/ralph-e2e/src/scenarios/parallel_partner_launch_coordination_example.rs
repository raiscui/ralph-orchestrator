...
//!
//! 目标:
//! - 直接覆盖 `examples/parallel-partner-launch-coordination`
//! - 验证 partner launch 多输入线并行推进后由 finalizer 汇总 `partner.launch.ready`
...
/// 直接覆盖 `examples/parallel-partner-launch-coordination` 的端到端场景。
pub struct ParallelPartnerLaunchCoordinationExampleScenario {
    id: String,
    description: String,
    tier: String,
}

impl ParallelPartnerLaunchCoordinationExampleScenario {
    pub fn new() -> Self {
        Self {
            id: "parallel-partner-launch-coordination-example".to_string(),
            description: "Directly runs examples/parallel-partner-launch-coordination and asserts partner lanes converge into partner.launch.ready".to_string(),
    ...
    fn parallel_mode_visible(&self, result: &ExecutionResult) -> crate::models::Assertion {
        ...
        let builder =
            AssertionBuilder::new("Parallel mode visible (partner launch coordination example)")
    ...
    fn agents_snapshot_written(&self, executor: &RalphExecutor) -> crate::models::Assertion {
        let snapshot = match read_agents_snapshot(executor.workspace()) {
            ...
            Err(error) => {
                return AssertionBuilder::new(
                    "Agents snapshot written (partner launch coordination example)",
                )
        ...
        let has_solution = snapshot
            .instances
            .iter()
            .any(|instance| instance.hat_id == "solution_enablement_lead");
        let has_legal = snapshot
            .instances
            .iter()
            .any(|instance| instance.hat_id == "legal_terms_owner");
        let has_marketing = snapshot
            .instances
            .iter()
            .any(|instance| instance.hat_id == "channel_marketing_manager");
        let has_sales = snapshot
            .instances
            .iter()
            .any(|instance| instance.hat_id == "sales_handoff_manager");
        let has_launch_manager = snapshot
            .instances
            .iter()
            .any(|instance| instance.hat_id == "partner_launch_manager");
        ...
        .expected("agents.json contains 4 lane hats + partner launch manager")
        .actual(format!(
            "instance_count={instance_count}, solution={has_solution}, legal={has_legal}, marketing={has_marketing}, sales={has_sales}, launch_manager={has_launch_manager}"
        ));
    ...
    fn required_topic_chain_observed(&self, result: &ExecutionResult) -> crate::models::Assertion {
        ...
        let builder = AssertionBuilder::new(
            "Required topic chain observed (partner launch coordination example)",
        )
        .expected("all partner launch lane topics + partner.launch.ready are present")
    ...
    fn no_unexpected_gates(&self, result: &ExecutionResult) -> crate::models::Assertion {
        ...
        let builder =
            AssertionBuilder::new("No unexpected gates (partner launch coordination example)")
    ...
    fn final_payload_expected(&self, result: &ExecutionResult) -> crate::models::Assertion {
        let payload = result
            ...
            .iter()
            .rev()
            .find(|event| event.topic == "partner.launch.ready")
            .map(|event| event.payload.clone())
            .unwrap_or_default();

        let ok = partner_launch_payload_matches(&payload);
        let builder = AssertionBuilder::new(
            "Final payload expected (partner launch coordination example)",
        )
        .expected(
            "partner.launch.ready payload semantically contains READY_TO_ANNOUNCE, NORTH_AMERICA, and channel_enablement",
        )
        .actual(if payload.is_empty() {
            "partner.launch.ready payload missing".to_string()
        } else {
            payload
        });
    ...
    fn no_new_jobs_started_after_loop_complete(
        ...
        let builder = AssertionBuilder::new(
            "No new jobs after LOOP_COMPLETE (partner launch coordination example)",
        )
...
fn partner_launch_payload_matches(payload: &str) -> bool {
    if payload.is_empty() {
        return false;
    }
    ...
    if let Ok(value) = serde_json::from_str::<serde_json::Value>(payload) {
        let status = value
            .get("partner_launch_status")
            .and_then(|value| value.as_str());
        let region = value.get("launch_region").and_then(|value| value.as_str());
        let checkpoint = value
            .get("next_checkpoint")
            .and_then(|value| value.as_str());

        return status == Some("READY_TO_ANNOUNCE")
            && region == Some("NORTH_AMERICA")
            && checkpoint == Some("channel_enablement");
    }

    payload.contains("partner_launch_status: READY_TO_ANNOUNCE")
        && payload.contains("launch_region: NORTH_AMERICA")
        && payload.contains("next_checkpoint: channel_enablement")
}

impl Default for ParallelPartnerLaunchCoordinationExampleScenario {
    fn default() -> Self {
        Self::new()
    }
}
