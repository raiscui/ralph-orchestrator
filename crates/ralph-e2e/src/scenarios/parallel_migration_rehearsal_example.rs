...
//!
//! 目标:
//! - 直接覆盖 `examples/parallel-migration-rehearsal`
//! - 验证 rehearsal lane fanout、go / no-go 汇总与 migration.ready 闭环
...
/// 直接覆盖 `examples/parallel-migration-rehearsal` 的端到端场景。
pub struct ParallelMigrationRehearsalExampleScenario {
    id: String,
    description: String,
    tier: String,
}

impl ParallelMigrationRehearsalExampleScenario {
    pub fn new() -> Self {
        Self {
            id: "parallel-migration-rehearsal-example".to_string(),
            description: "Directly runs examples/parallel-migration-rehearsal and asserts rehearsal lanes converge into migration.ready".to_string(),
    ...
    fn parallel_mode_visible(&self, result: &ExecutionResult) -> crate::models::Assertion {
        ...
        let builder = AssertionBuilder::new("Parallel mode visible (migration rehearsal example)")
    ...
    fn agents_snapshot_written(&self, executor: &RalphExecutor) -> crate::models::Assertion {
        let snapshot = match read_agents_snapshot(executor.workspace()) {
            ...
            Err(error) => {
                return AssertionBuilder::new(
                    "Agents snapshot written (migration rehearsal example)",
                )
        ...
        let has_schema = snapshot
            .instances
            .iter()
            .any(|instance| instance.hat_id == "schema_reviewer");
        let has_backup = snapshot
            .instances
            .iter()
            .any(|instance| instance.hat_id == "backup_verifier");
        let has_smoke = snapshot
            .instances
            .iter()
            .any(|instance| instance.hat_id == "smoke_runner");
        let has_rollback = snapshot
            .instances
            .iter()
            .any(|instance| instance.hat_id == "rollback_auditor");
        let has_conductor = snapshot
            .instances
            .iter()
            .any(|instance| instance.hat_id == "migration_conductor");
        ...
        let builder =
            AssertionBuilder::new("Agents snapshot written (migration rehearsal example)")
                .expected("agents.json contains 4 lane hats + migration conductor")
                .actual(format!(
                    "instance_count={instance_count}, schema={has_schema}, backup={has_backup}, smoke={has_smoke}, rollback={has_rollback}, conductor={has_conductor}"
                ));
    ...
    fn required_topic_chain_observed(&self, result: &ExecutionResult) -> crate::models::Assertion {
        ...
        let builder =
            AssertionBuilder::new("Required topic chain observed (migration rehearsal example)")
                .expected("all rehearsal lane topics + migration.ready are present")
    ...
    fn no_unexpected_gates(&self, result: &ExecutionResult) -> crate::models::Assertion {
        ...
        let builder = AssertionBuilder::new("No unexpected gates (migration rehearsal example)")
    ...
    fn final_payload_expected(&self, result: &ExecutionResult) -> crate::models::Assertion {
        let payload = result
            ...
            .iter()
            .rev()
            .find(|event| event.topic == "migration.ready")
            .map(|event| event.payload.clone())
            .unwrap_or_default();

        let ok = payload.contains("decision: GO")
            || (payload.contains("GO") && payload.contains("15m_canary_then_full"));
        let builder = AssertionBuilder::new("Final payload expected (migration rehearsal example)")
            .expected("migration.ready payload contains decision GO and rollout plan")
            .actual(if payload.is_empty() {
                "migration.ready payload missing".to_string()
            } else {
                payload
            });
    ...
    fn no_new_jobs_started_after_loop_complete(
        ...
        let builder =
            AssertionBuilder::new("No new jobs after LOOP_COMPLETE (migration rehearsal example)")
...
impl Default for ParallelMigrationRehearsalExampleScenario {
    fn default() -> Self {
        Self::new()
    }
}
...
impl TestScenario for ParallelMigrationRehearsalExampleScenario {
    ...
    fn setup(&self, workspace: &Path, backend: Backend) -> Result<ScenarioConfig, ScenarioError> {
        setup_prompt_file_example_workspace(workspace, backend, "parallel-migration-rehearsal", 18)
    }
