...
//!
//! 目标:
//! - 直接覆盖 `examples/parallel-support-escalation-desk`
//! - 验证 support escalation 多输入线并行推进后由 finalizer 汇总 `escalation.plan.ready`
...
/// 直接覆盖 `examples/parallel-support-escalation-desk` 的端到端场景。
pub struct ParallelSupportEscalationDeskExampleScenario {
    id: String,
    description: String,
    tier: String,
}

impl ParallelSupportEscalationDeskExampleScenario {
    pub fn new() -> Self {
        Self {
            id: "parallel-support-escalation-desk-example".to_string(),
            description: "Directly runs examples/parallel-support-escalation-desk and asserts escalation lanes converge into escalation.plan.ready".to_string(),
    ...
    fn parallel_mode_visible(&self, result: &ExecutionResult) -> crate::models::Assertion {
        ...
        let builder =
            AssertionBuilder::new("Parallel mode visible (support escalation desk example)")
    ...
    fn agents_snapshot_written(&self, executor: &RalphExecutor) -> crate::models::Assertion {
        let snapshot = match read_agents_snapshot(executor.workspace()) {
            ...
            Err(error) => {
                return AssertionBuilder::new(
                    "Agents snapshot written (support escalation desk example)",
                )
        ...
        let has_case = snapshot
            .instances
            .iter()
            .any(|instance| instance.hat_id == "case_triage_owner");
        let has_product = snapshot
            .instances
            .iter()
            .any(|instance| instance.hat_id == "product_bug_reviewer");
        let has_account = snapshot
            .instances
            .iter()
            .any(|instance| instance.hat_id == "account_risk_manager");
        let has_comms = snapshot
            .instances
            .iter()
            .any(|instance| instance.hat_id == "comms_coordinator");
        let has_director = snapshot
            .instances
            .iter()
            .any(|instance| instance.hat_id == "escalation_director");
        ...
        let builder =
            AssertionBuilder::new("Agents snapshot written (support escalation desk example)")
                .expected("agents.json contains 4 lane hats + escalation director")
                .actual(format!(
                    "instance_count={instance_count}, case={has_case}, product={has_product}, account={has_account}, comms={has_comms}, director={has_director}"
                ));
    ...
    fn required_topic_chain_observed(&self, result: &ExecutionResult) -> crate::models::Assertion {
        ...
        let builder = AssertionBuilder::new(
            "Required topic chain observed (support escalation desk example)",
        )
        .expected("all escalation lane topics + escalation.plan.ready are present")
    ...
    fn no_unexpected_gates(&self, result: &ExecutionResult) -> crate::models::Assertion {
        ...
        let builder =
            AssertionBuilder::new("No unexpected gates (support escalation desk example)")
    ...
    fn final_payload_expected(&self, result: &ExecutionResult) -> crate::models::Assertion {
        let payload = result
            ...
            .iter()
            .rev()
            .find(|event| event.topic == "escalation.plan.ready")
            .map(|event| event.payload.clone())
            .unwrap_or_default();

        let ok = escalation_payload_matches(&payload);
        let builder =
            AssertionBuilder::new("Final payload expected (support escalation desk example)")
                .expected(
                    "escalation.plan.ready payload semantically contains READY_FOR_EXECUTION, SEV_2, and support-director",
                )
                .actual(if payload.is_empty() {
                    "escalation.plan.ready payload missing".to_string()
                } else {
                    payload
                });
    ...
    fn no_new_jobs_started_after_loop_complete(
        ...
        let builder = AssertionBuilder::new(
            "No new jobs after LOOP_COMPLETE (support escalation desk example)",
        )
...
fn escalation_payload_matches(payload: &str) -> bool {
    if payload.is_empty() {
        return false;
    }

    if let Ok(value) = serde_json::from_str::<serde_json::Value>(payload) {
        let escalation_status = value
            .get("escalation_status")
            .and_then(|value| value.as_str());
        let severity = value.get("severity").and_then(|value| value.as_str());
        let next_update_owner = value
            .get("next_update_owner")
            .and_then(|value| value.as_str());

        return escalation_status == Some("READY_FOR_EXECUTION")
            && severity == Some("SEV_2")
            && next_update_owner == Some("support-director");
    }

    payload.contains("escalation_status: READY_FOR_EXECUTION")
        && payload.contains("severity: SEV_2")
        && payload.contains("next_update_owner: support-director")
}

impl Default for ParallelSupportEscalationDeskExampleScenario {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl TestScenario for ParallelSupportEscalationDeskExampleScenario {
    fn id(&self) -> &str {
        &self.id
    }
...
mod tests {
    ...
    fn example_config_does_not_embed_raw_event_blocks() {
        let config =
            include_str!("../../../../examples/parallel-support-escalation-desk/ralph.yml");
    ...
    fn example_config_requires_silent_wait_before_all_ready_lanes() {
        ...
        assert!(
            ...
            "parallel-support-escalation-desk config must explicitly forbid interim prose before all ready lanes arrive"
        );
    }
    ...
    fn example_config_forbids_self_closing_events() {
        ...
        assert!(
            ...
            "support escalation example must forbid self-closing events and attribute-only payloads"
        );
    }

    #[test]
    fn payload_matcher_accepts_json_and_line_payloads() {
        let json_payload = r#"{"case_id":"ESC-2049","escalation_status":"READY_FOR_EXECUTION","severity":"SEV_2","next_update_owner":"support-director"}"#;
        let line_payload = "case_id: ESC-2049
escalation_status: READY_FOR_EXECUTION
severity: SEV_2
next_update_owner: support-director";

        assert!(super::escalation_payload_matches(json_payload));
        assert!(super::escalation_payload_matches(line_payload));
    }
}
