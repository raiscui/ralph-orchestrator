...
## 2026-02-11 20:52 +0800 | 并行 Ralph: Codex MCP 常驻会话 + 忙时切换 ralph#2

### 需求收敛

- `ralph#1` 的 Codex 不再一轮一启停,而是常驻 `codex mcp-server`.
- 当事件要交给 `ralph#1` 且其处于 Running 时,自动将事件交给 `ralph#2`.
- TUI chat 不走旁路,继续通过现有 `human.message` 事件链路.

### 路由层实现要点

- 文件: `crates/ralph-core/src/parallel/supervisor/routing.rs`
- 新增 `rewrite_target_for_busy_ralph`:
  - 若 `target_instance=ralph#1` 且主实例 Running,重写为 `ralph#2`.
- 新增 `choose_ralph_instance_for_delivery`:
  - `ralph#1` 非 Running -> 选 `ralph#1`
  - `ralph#1` Running 且 `ralph#2` 非 Running -> 选 `ralph#2`
  - 双 Running -> 回退 `ralph#1`(交由 pending 队列吸收)
- `escalate_delivery_failure` 也改为通过该分配器找可用 ralph 实例.

### 执行器实现要点

- 新文件: `crates/ralph-cli/src/codex_mcp_session.rs`
  - 新增 `CodexMcpRuntime` + `CodexMcpSession`.
  - 每个实例(`ralph#1`/`ralph#2`)懒启动并复用一个 `codex mcp-server` 子进程.
  - 首轮调用工具 `codex`,后续基于 `threadId` 调用 `codex-reply`.
  - 监听 `codex/event` 中 `agent_message_delta`,流式推送到 TUI.
  - `ralph#2` 首次接管时注入 `ralph#1` 最近摘要,但 thread 独立.
- 文件: `crates/ralph-cli/src/parallel_runner.rs`
  - 仅 `ralph#1`/`ralph#2` + backend=codex 走 MCP 常驻路径.
  - 运行结束/中断/错误都执行 `shutdown_all` 回收会话进程.

### 测试与验证

- 路由测试新增:
  - `busy_ralph_primary_explicit_target_is_redirected_to_secondary`
