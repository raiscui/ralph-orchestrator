...
## 2026-02-03 16:45 +0800｜Hat Graph Radar：Running 高亮色 #a9dc76 + 并行 event.source 补齐（边动画可触发）
- `crates/ralph-tui/src/theme.rs` / `crates/ralph-tui/src/app.rs`：Running hat box 高亮改为 `#a9dc76`
- `crates/ralph-core/src/parallel/instance.rs`：outgoing event 自动补齐 `source=hat_id`（修复边动画触发条件）
- 验证：`cargo fmt --check`、`cargo clippy --all-targets --all-features -- -D warnings`、`cargo test`、`cargo test -p ralph-core smoke_runner` ✅

---

## 2026-02-03 15:12 +0800｜tui hat graph radar：Running hats 蓝色高亮 + 最新 event 边动画（逐段点亮）

### 目标
- Radar 字符图里：所有“运行中”的 hat box 前景蓝色。
- 根据最新 event：把“发布者 hat → 订阅者 hats”的边做动画。
  动画风格为“从起点到终点逐段点亮”（progressive reveal）。

### 关键结论（架构）
- Ralph 侧的 Radar 渲染不是 exec `beautiful-mermaid-rs` CLI。
  它是直接调用 `beautiful-mermaid-rs` crate 的渲染函数。
- 因为要做“box 高亮 + 线段动画”，仅在 Rust 侧对最终字符串做解析会非常脆弱。
  正确姿势是让渲染器输出坐标 meta（node/edge/path），TUI 再做 cell-level overlay。

### 我做了什么

- TS（`/Users/cuiluming/local_doc/l_dev/ref/typescript/beautiful-mermaid`）
  - 新增 `renderMermaidAsciiWithMeta(...) -> { text, meta }`。
  - meta 提供 nodes 的 box bounds + edges 的 stroke path（有序坐标序列）。
  - 新增回归测试，确保 text 不变且仍可反向解析回 Mermaid（逻辑一致）。

- Rust（`/Users/cuiluming/local_doc/l_dev/my/rust/beautiful-mermaid-rs`）
  - 同步 vendor bundle（把新 API 带进 QuickJS）。
  - 新增 `render_mermaid_ascii_with_meta(...) -> AsciiRenderWithMeta`（text + meta）。
  - 新增 smoke test，确保 meta 非空且 text 与旧 API 一致。

- Ralph（本仓库）
  - `crates/ralph-cli/src/hats.rs`
    - Radar 渲染从 `render_mermaid_ascii` 升级为 `render_mermaid_ascii_with_meta`。
    - 启动 TUI 时注入 `ascii_compact/ascii_full + meta_compact/meta_full`。
  - `crates/ralph-tui/src/state.rs`
    - 增加 Radar meta 结构体与 `hat_graph_animation` 状态。
    - 并行/串行事件到来时，若 event 带 source，则记录动画触发信息。
  - `crates/ralph-tui/src/app.rs`
    - 渲染 Radar 后对 buffer 做叠加渲染：
      - Running hats：box 前景蓝色（并行模式高亮所有 Running）。
      - 最新 event：匹配 edge.label=topic 且 from=sourceHat 的边，按 path 逐段点亮。
...
## 2026-02-03 13:44 +0800｜微调：Radar 再向下偏移，避免遮挡 Output 边线

### 背景
- Radar 之前的纵向 inset 为 2 行，会与并行模式 Output 面板的 top border（通常在 y=2）重叠，导致边线被覆盖。
- 你希望 Radar 再向下偏移一点，确保 Output 边线完整可见。

### 我做了什么
- `crates/ralph-tui/src/app.rs`
  - `HAT_GRAPH_RADAR_INSET_Y: 2 → 3`
  - 保持 `HAT_GRAPH_RADAR_INSET_X = 2` 不变（右侧留白仍然存在）。
  - 同步更新回归测试：由于 inset_y 增大，clamp 场景下可用高度减少，期望值相应调整。
- `specs/terminal-ui.spec.md`
  - 强化说明：Radar inset 不仅是“视觉留白”，也用于避免覆盖关键 pane 边框（例如 Output top border）。
...
## 2026-02-03 13:16 +0800｜更正：TUI Hat Graph Radar 向内偏移（从右上角往左下移）

### 背景
- 需求澄清：你说的“偏移”指的是 **向内留白**（往左下移），而不是把 Radar 顶到右上角贴边。

### 我做了什么
- `crates/ralph-tui/src/app.rs`
  - 新增 Radar 向内偏移常量：
    - `HAT_GRAPH_RADAR_INSET_X = 2`（右侧留白：向左偏移 2 列）
    - `HAT_GRAPH_RADAR_INSET_Y = 2`（顶部留白：向下偏移 2 行）
  - `hat_graph_radar_area(...)`：
    - 先用 `bounds - inset` 计算“可用空间”（`available_width/height`），过小则隐藏（避免越界）。
    - 面板尺寸仍优先按“字符图尺寸 + 边框”自适配，但会被可用空间裁剪。
    - 最终坐标用“右上角（减去 inset）”作为锚点，实现“从右上角向内偏移”的位置。
  - 回归测试同步更新：从期望 `y=0` 调整为期望 `y=HAT_GRAPH_RADAR_INSET_Y`，并同步 x/width/height 的 clamp 预期。
- `specs/terminal-ui.spec.md`
...
## 2026-02-03 12:55 +0800｜修正：TUI Hat Graph Radar 贴右上角 + Zoom 尺寸自适配字符图

### 改动点
- `crates/ralph-tui/src/app.rs`
  - Radar 的锚点 bounds 从 `content_area` 改为“整屏去掉 footer”，让面板 y=0，视觉上真正贴到右上角（不再空 2 行）。
  - zoom 尺寸改为按“字符图实际宽高 + 边框”自适配，并在 bounds 内裁剪（尽量避免裁切与过大留白）。
  - mini 仍保留雷达上限（<=36x10），但当字符图更小时会自动收缩，减少无意义留白。
  - 新增回归测试：Unicode display width 测量、area 锚点 y=0、zoom 自适配与 clamp。
- `specs/terminal-ui.spec.md`
  - 更新 Radar 的锚点语义（frame 去掉 footer）。
  - 补充 zoom 尺寸 SHOULD 自适配字符图尺寸的规则。
...
## 2026-02-03 16:21 +0800｜Bugfix：并行模式 Hat Graph Radar 高亮闪烁 & event 边动画不触发

### 问题
- Running hat 的 box 高亮会“闪一下就没了”（状态回退）。
- 最新 event 的边动画完全不触发（UI 没收到可用事件）。

### 修复
- `crates/ralph-tui/src/state/parallel.rs`
  - `append_output()` 仅在实例不存在时注册 Created，避免覆盖 Running/Idle/Done 状态。
- `crates/ralph-cli/src/parallel_runner.rs`
  - 并行模式把 `gate.*` / `human.message` / “带 source 的业务事件”转发进 TUI（用于触发边动画）。
  - 抽出 `should_forward_event_to_tui()` 并加单测锁死策略。
